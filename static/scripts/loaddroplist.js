...
dropNumberContainer.className = 'drop-number-container';
...
if (isUpcoming) {
    ...
    dropLink.classList.add('drop-upcoming');
...
function createLinkElement(text, url) {
    ...
    function addHoverEffectToLinksForTitles(container, allowedTitles) {
        ...
        container.addEventListener('mouseover', (event) => {
            if (event.target.classList.contains('drop-link')) {
                const section = event.target.closest('.drop-section'); // Finde die übergeordnete Section
    ...
    if (distance < 0) {
        ...
        function addNewsletterInput(nextDropNumber) {
            ...
            newsletterContainer.className = 'drop-item';
            newsletterContainer.classList.add('active-drop');

            const dropNumberContainer = document.createElement('div');
            dropNumberContainer.className = 'drop-number-container';
            dropNumberContainer.classList.add('newsletter-number-container')
            ...
            newsletterLabel.className = 'drop-link';
            newsletterLabel.textContent = 'ENTER EMAIL:';

            const emailInput = document.createElement('input');
            emailInput.id = 'email';
            emailInput.className = 'drop-link';
            emailInput.setAttribute('type', 'email');
            emailInput.setAttribute('placeholder', '');

            const submitButton = document.createElement('div');
            submitButton.className = 'submit-button hidden';
...
buttonContainer.className = 'drop-item';
...
collectionNumberContainer.className = 'drop-number-container';
...
collectionButton.className = 'drop-link';
...
collections.forEach((collection) => {
    ...
    if (!collection.handle.includes('drop')) {
        ...
        collectionLink.className = 'drop-link';
        ...
        collectionContainer.className = 'drop-item';
        ...
        placeholderNumberContainer.className = 'drop-number-container';
